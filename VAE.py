        ...
        for file in os.listdir(directory):
            ...
            for i in range(songroll.shape[0]//96): # Split song up into 4 beat segments i.e. 1 bar 
                ...
                if len(batch) >= batch_size:
                    dataset.append(np.transpose(np.array(batch), (1, 0, 2)).astype('float32')) # Transpose so batch is 2nd dimension
                    batch = []
...
class Optimisation:
    def __init__(self, model, optimiser, beta):
        ...
        self.mse = nn.MSELoss()
        self.beta = beta
        self.genredict = {"Blues": 0, "Country": 1, "Jazz": 2, "Rock": 3, "Pop": 4}

    def loss_fn(self, x, x_hat, mu, logvar, z, genre):
    ...
    def train_step(self, x, genre, vae=True):
        self.optimiser.zero_grad()
        self.model.train()
        if vae:
            x_hat, mu, logvar, z = self.model(x)
            loss = self.loss_fn(x, x_hat, mu, logvar, z, genre)
        loss.backward()
        self.optimiser.step()
        return loss.item()
    
    def test_step(self, x, genre):
        x_hat, mu, logvar, z = self.model(x)
        loss = self.loss_fn(x, x_hat, mu, logvar, z, genre)
        return loss.item()
        ...
        for epoch in range(1, n_epochs+1):
            if epoch % eval_every == 0:
                ...
                for batch in test_set:
                    ...
                    loss = self.test_step(input, genre)
